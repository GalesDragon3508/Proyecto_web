let enemyFlySpeed = 200; // Velocidad de los enemigos voladores
let enemyFlyHealth = 3; // Salud de los enemigos voladores
let groupFlyEnemies; // Grupo de enemigos voladores

const FLY_ENEMY_PROBABILITY = 0.5;

let partBState = {
    preload: loadPartBAssets,
    create: doPartB,
    update: updatePlayPartB
};
function loadImage(){
    game.load.image('player', 'assets/imgs/Jugador2.png');
...
function loadSprite(){
    ...
    game.load.spritesheet('collectcoin', 'assets/imgs/Moneda.png', 25, 40, 1);
    game.load.spritesheet('flyAlien', 'assets/imgs/alienAnimacion.png', 50, 60, 4);
}
...
function updatePlayPartB() {
    ...
    game.physics.arcade.overlap(groupBullets, groupFlyEnemies, BalaHitFlyEnemy, null, this);
    game.physics.arcade.collide(player, groupFlyEnemies, FlyEnemyHitPlayer, null, this);
...
function spawnEnemies(){
    if(enemy_count<spawn_limit){
        ...
        if (Math.random() < FLY_ENEMY_PROBABILITY) {
            game.time.events.add(createFlyEnemies(1), this); // Añade 1 enemigo volador por cada llamada con probabilidad
        }
    }
}
...
function createFlyEnemies(numb) {
    groupFlyEnemies = game.add.group();
    groupFlyEnemies.enableBody = true;
    groupFlyEnemies.createMultiple(numb, 'flyAlien');
    groupFlyEnemies.callAll('events.onOutOfBounds.add', 'events.onOutOfBounds', resetMember);
    groupFlyEnemies.callAll('anchor.setTo', 'anchor', 0.5, 0.5);
    groupFlyEnemies.setAll('checkWorldBounds', true);

    groupFlyEnemies.forEach(function(flyEnemy) {
        flyEnemy.health = enemyFlyHealth;
        flyEnemy.animations.add('fly', [0, 1, 2, 3], 10, true);
        flyEnemy.animations.play('fly');
    });
}

function updateFlyEnemyMovements() {
    groupFlyEnemies.forEachAlive(function(flyEnemy) {
        game.physics.arcade.moveToObject(flyEnemy, player, enemyFlySpeed);
    });
}

function BalaHitFlyEnemy(bala, flyEnemy) {
    flyEnemy.health -= 1;
    bala.kill();
    if (flyEnemy.health <= 0) {
        let x = flyEnemy.x;
        let y = flyEnemy.y;
        flyEnemy.kill();
        muerteEnemigo.play();
...
function FlyEnemyHitPlayer(player, flyEnemy) {
    flyEnemy.kill();
    lifeCounter--;
    if (lifeCounter <= 0) {
        defeat();
